...
io.on('connection', function(socket){
    ...
    socket.on('send-nickname', function(data,whiteboardId) {
        socket.roomname = data.roomname;
        socket.username = data.username;

        if(!userExists(socket.roomname,socket.username)){
            arr.push({id:socket.roomname,username:socket.username})
            socket.broadcast.emit('updateUser',arr);
            socket.broadcast.emit('fetchUser',arr);
        } else{
            socket.broadcast.emit('updateUser',arr);
            socket.broadcast.emit('fetchUser',arr);
        }
...
function userExists(id,username) {
    ...
    return arr.some(function(el) {
        console.log("Conditation "+(el.username === username && el.id === id));
